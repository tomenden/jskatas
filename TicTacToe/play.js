...
TicTacToe.prototype.get_possibleMoves = function (board) {
    var possibleMoves = [];
    for (var i = 0; i < 3; i += 1) {
        for (var j = 0; j < 3; j += 1) {
            if (board[i][j] === '-')
...
TicTacToe.prototype.calculateScore = function(board) {
    var lines = [];
    lines.push(board[0]);
    lines.push(board[1]);
    lines.push(board[2]);
...
TicTacToe.prototype.get_possibleStates = function(player, board) {
